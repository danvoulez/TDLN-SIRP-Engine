...
- **Put**: `/registry/put` with `{name, version, cid}` (idempotent; append audit entry).
- **Storage**: provider-agnostic (S3-compatible URIs); engine holds only metadata references.
- **Lookups**: by `{name, version}` → CID; deterministic responses.
