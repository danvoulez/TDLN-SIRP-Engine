...
pub enum ExprSpec {
    ...
    Unary {
        operator: String,
        argument: Box<ExprSpec>,
    },
    FunctionCall {
        function: String,
        arguments: Vec<ExprSpec>,
    },
    Conditional {
        test: Box<ExprSpec>,
        consequent: Box<ExprSpec>,
        alternate: Box<ExprSpec>,
    },
}
...
fn expr_from_spec(s: &ExprSpec) -> Result<Expression> {
    use Expression as E;
    Ok(match s {
        ExprSpec::Literal { value } => E::Literal {
            value: value.clone(),
        },
        ExprSpec::ContextRef { path } => E::ContextRef {
            path: path.clone(),
            fallback: None,
        },
        ExprSpec::Binary {
        ...
        ExprSpec::Conditional {
            ...
            test: Box::new(expr_from_spec(test)?),
            consequent: Box::new(expr_from_spec(consequent)?),
            alternate: Box::new(expr_from_spec(alternate)?),
        },
    })
}
...
fn wiring_from_spec(w: &WiringSpec) -> Result<Wiring> {
    Ok(match w {
        WiringSpec::All { policies } => Wiring::All {
            policies: policies.clone(),
        },
        WiringSpec::Any { policies } => Wiring::Any {
            policies: policies.clone(),
        },
        WiringSpec::Majority { policies } => Wiring::Majority {
            policies: policies.clone(),
        },
        WiringSpec::Sequential { policies } => Wiring::Sequential {
            policies: policies.clone(),
        },
        WiringSpec::Weighted {
            policies,
...
impl UnitStore {
    ...
    pub fn list(&self) -> Vec<AtomicUnit> {
        self.inner.read().clone()
    }
    pub fn get(&self, id: &str) -> Option<AtomicUnit> {
        self.inner.read().iter().find(|u| u.id == id).cloned()
    }
    pub fn replace_all(&self, units: Vec<AtomicUnit>) {
        *self.inner.write() = units;
    }
}
...
pub async fn watch_units(store: UnitStore) -> Result<()> {
    ...
    // initial load
    if let Ok(u) = load_units_from_dir(&dir).await {
        store.replace_all(u)
    }
