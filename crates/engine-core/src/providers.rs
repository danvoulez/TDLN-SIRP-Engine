...
pub trait CanonProvider: Send + Sync {
    fn canon(&self, v: &Json) -> Vec<u8>;
}
pub trait CidProvider: Send + Sync {
    fn cid(&self, bytes: &[u8]) -> String;
}
...
impl DefaultExpr {
    ...
    fn as_number(v: &Json) -> Result<f64> {
        match v {
            ...
            Json::String(s) => s
                .parse::<f64>()
                .map_err(|_| anyhow!("Cannot parse as number: {s}")),
        ...
        match expr {
            ...
            ContextRef { path, fallback } => {
                ...
                for k in path {
                    match cur.get(k) {
                        Some(v) => cur = v,
                        None => return Ok(fallback.clone().unwrap_or(Json::Null)),
                    }
                }
                Ok(cur.clone())
            }
            Binary {
                ...
                Ok(match operator {
                    ...
                    In => match &r {
                        ...
                        Json::String(s) => {
                            Json::Bool(l.as_str().map(|t| s.contains(t)).unwrap_or(false))
                        }
                        _ => Json::Bool(false),
                    },
                ...
                match function.as_str() {
                    "length" => {
                        if let Some(s) = args.first().and_then(|v| v.as_str()) {
                            Json::Number((s.len() as u64).into())
                        } else if let Some(a) = args.first().and_then(|v| v.as_array()) {
                            Json::Number((a.len() as u64).into())
                        } else {
            ...
            Conditional {
                ...
                if Self::as_bool(&self.eval(test, ctx)?)? {
                    self.eval(consequent, ctx)
                } else {
                    self.eval(alternate, ctx)
                }
            }
        }
    }
...
trait Pipe<T> {
    fn pipe<F: FnOnce(T) -> R, R>(self, f: F) -> R;
}
impl<T> Pipe<T> for T {
    fn pipe<F: FnOnce(T) -> R, R>(self, f: F) -> R {
        f(self)
...
impl AggregatorStrategy for DefaultAggregator {
    fn aggregate(&self, wiring: &Wiring, decisions: &[PolicyDecision]) -> Decision {
        use Decision::*;
        let map: std::collections::HashMap<_, _> = decisions
            .iter()
            .filter(|d| !d.skipped)
            .map(|d| (d.policy_id.clone(), d.decision.clone()))
            .collect();
        match wiring {
            Wiring::All { policies } => {
                ...
                if rel.iter().all(|d| **d == Allow) {
                    Allow
                } else if rel.iter().any(|d| **d == Doubt) {
                    Doubt
                } else {
                    Deny
                }
...
impl ReceiptSink for NoopSink {
    fn emit(&self, _: &crate::model::ExecutionReceipt) -> Result<()> {
        Ok(())
    }
}
