...
where
    G: IdGen + Default,
    E: ExprEval + Default,
    A: AggregatorStrategy + Default,
    CX: CanonProvider + Default,
    CD: CidProvider + Default,
    S: Signer + Default,
    T: ReceiptSink + Default,
{
    fn default() -> Self {
        Self {
            ...
            expr: Some(E::default()),
            agg: Some(A::default()),
            canon: Some(CX::default()),
            cid: Some(CD::default()),
            signer: Some(S::default()),
            sink: Some(T::default()),
            clock: None,
        }
    }
}
impl Default for crate::providers::DefaultExpr {
    fn default() -> Self {
        Self
    }
}
impl Default for crate::providers::DefaultAggregator {
    fn default() -> Self {
        Self
    }
}
impl Default for crate::providers::DefaultCanon {
    fn default() -> Self {
        Self
    }
}
impl Default for crate::providers::DefaultCid {
    fn default() -> Self {
        Self
    }
}
impl Default for crate::providers::NoopSigner {
    fn default() -> Self {
        Self
    }
}
impl Default for crate::providers::NoopSink {
    fn default() -> Self {
        Self
    }
}
impl Default for crate::providers::UlidGen {
    fn default() -> Self {
        Self
    }
}
...
where
    ...
    T: ReceiptSink,
{
    pub fn chip(mut self, c: SemanticChip) -> Self {
        self.chips.insert(c.id.clone(), c);
        self
    }
    pub fn chips(mut self, v: Vec<SemanticChip>) -> Self {
        for c in v {
            self.chips.insert(c.id.clone(), c);
        }
        self
    }
    pub fn default_mode(mut self, m: EngineMode) -> Self {
        self.default_mode = Some(m);
        self
    }
    pub fn id(mut self, v: G) -> Self {
        self.id = Some(v);
        self
    }
    pub fn expr(mut self, v: E) -> Self {
        self.expr = Some(v);
        self
    }
    pub fn agg(mut self, v: A) -> Self {
        self.agg = Some(v);
        self
    }
    pub fn canon(mut self, v: CX) -> Self {
        self.canon = Some(v);
        self
    }
    pub fn cid(mut self, v: CD) -> Self {
        self.cid = Some(v);
        self
    }
    pub fn signer(mut self, v: S) -> Self {
        self.signer = Some(v);
        self
    }
    pub fn sink(mut self, v: T) -> Self {
        self.sink = Some(v);
        self
    }
    pub fn clock(mut self, v: Box<dyn Clock>) -> Self {
        self.clock = Some(v);
        self
    }

    pub fn defaults() -> EngineBuilder<
        crate::providers::UlidGen,
        crate::providers::DefaultExpr,
        crate::providers::DefaultAggregator,
        crate::providers::DefaultCanon,
        crate::providers::DefaultCid,
        crate::providers::NoopSigner,
        crate::providers::NoopSink,
    > {
    ...
    pub fn build(self) -> Engine<G, E, A, CX, CD, S, T> {
        Engine {
            ...
            clock: self
                .clock
                .unwrap_or_else(|| Box::new(crate::providers::SysClock)),
        }
    }
}
    ...
    T: ReceiptSink,
{
    pub fn execute(
        ...
        let to_sign = self
            .canon
            .canon(&json!({ "input": input_cid, "output": output_cid, "hash_chain": hash_chain }));
...
pub fn denied_receipt(
    ...
    ExecutionReceipt {
        ...
        input: CanonSlot {
            raw: input,
            canon: serde_json::from_slice(&canon).unwrap(),
            cid: input_cid.clone(),
        },
        policy_decisions: vec![],
        output: CanonSlot {
            raw: out,
            canon: serde_json::from_slice(&out_canon).unwrap(),
            cid: out_cid.clone(),
        },
        decision: Decision::Deny,
        missing: None,
        proof: Proof {
            hash_chain: vec![input_cid, out_cid],
            signature: None,
        },
