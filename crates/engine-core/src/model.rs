...
impl Expression {
    pub fn literal<T: Into<Json>>(v: T) -> Self {
        Self::Literal { value: v.into() }
    }
    pub fn context(path: &[&str]) -> Self {
        Self::ContextRef {
            path: path.iter().map(|s| s.to_string()).collect(),
            fallback: None,
        }
    }
    pub fn eq(l: Expression, r: Expression) -> Self {
        Self::Binary {
    ...
    pub fn gt(l: Expression, r: Expression) -> Self {
        Self::Binary {
...
impl Scope {
    pub fn permits(&self, path: &str) -> bool {
        fn m(p: &str, g: &str) -> bool {
            glob::Pattern::new(g)
                .map(|pat| pat.matches(p))
                .unwrap_or(false)
        }
        if self.deny.iter().any(|g| m(path, g)) {
            return false;
        }
...
impl EngineMode {
    ...
    pub fn allows(&self, eff: Effect) -> bool {
        self.enabled_effects.contains(&eff)
    }
    pub fn allows_all(&self, req: &[Effect]) -> bool {
        req.iter().all(|e| self.enabled_effects.contains(e))
    }
    pub fn is_policy_active(&self, id: &str) -> bool {
        self.active_policies.is_empty() || self.active_policies.contains(id)
    }
    pub fn permits(&self, eff: Effect, path: Option<&str>) -> bool {
        if !self.allows(eff.clone()) {
            return false;
        }
        if let Some(p) = path {
            if let Some(sc) = self.scopes.get(&eff) {
                return sc.permits(p);
            }
        }
        true
    ...
    pub fn is_public_safe(&self) -> bool {
        !self.enabled_effects.contains(&Effect::Process)
            && !self.enabled_effects.contains(&Effect::Wasm)
    }
}
...
impl PolicyBit {
    ...
    pub fn condition(mut self, e: Expression) -> Self {
        self.condition = e;
        self
    }
    pub fn requires(mut self, path: &[&str]) -> Self {
...
impl Wiring {
    pub fn ids(&self) -> Vec<String> {
        match self {
            ...
            | Wiring::Weighted { policies, .. } => policies.clone(),
        }
    }
}
...
impl SemanticChip {
    pub fn builder(id: &str) -> Builder {
        Builder {
            chip: Self {
                id: id.into(),
                name: None,
                policies: vec![],
                wiring: Wiring::All { policies: vec![] },
                required_effects: vec![],
                hash: None,
            },
        }
    }
    pub fn simple(id: &str, pols: Vec<PolicyBit>, wiring: Wiring) -> Self {
        Self {
            id: id.into(),
            name: None,
            policies: pols,
    ...
    pub fn with_required_effects(mut self, e: Vec<Effect>) -> Self {
        self.required_effects = e;
        self
    }
}
pub struct Builder {
...
impl Builder {
    pub fn name(mut self, n: &str) -> Self {
        self.chip.name = Some(n.into());
        self
    }
    pub fn policy(mut self, p: PolicyBit) -> Self {
        self.chip.policies.push(p);
        self
    }
    ...
    pub fn build(self) -> SemanticChip {
        self.chip
    }
}
