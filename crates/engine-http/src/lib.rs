...
pub fn engine_router(cfg: EngineHttpConfig) -> Router {
    let mut r = Router::new()
        .route(
            "/.well-known/logline/grl.json",
            axum::routing::get(well_known_grl),
        )
        ...
        .route(
            "/run",
            post(|| async { axum::Json(serde_json::json!({"receipt":"stub"})) }),
        )
        .route(
            "/registry/put",
        ...
        .route(
            "/acquire_presigned_url",
            post(|| async { axum::Json(serde_json::json!({"url":"stub"})) }),
        )
...
async fn emit_audit_report(
    ...
    let in_bytes = serde_json::to_vec(input_json)
        .map_err(|_| axum::http::StatusCode::INTERNAL_SERVER_ERROR)?;
    let out_bytes = serde_json::to_vec(output_json)
        .map_err(|_| axum::http::StatusCode::INTERNAL_SERVER_ERROR)?;
    ...
    let path = PathBuf::from(base_dir)
        .join("audit")
        .join(day)
        .join(format!("{}.json", report["audit_id"].as_str().unwrap()));
...
fn eval_presign_policy(req: &PresignReq) -> PolicyDecision {
    ...
    if !allowed_verbs.contains(&req.verb.as_str()) {
        return PolicyDecision::NACK {
            error: "verb_not_allowed".into(),
        };
    }
...
fn sign_grant(
    ...
    sk_b64: &str,
    kid: &str,
) -> Result<AccessGrant, axum::http::StatusCode> {
    ...
    let mut to_sign = grant.clone();
    to_sign.seal.sig = "".into();
    let bytes =
        serde_json::to_vec(&to_sign).map_err(|_| axum::http::StatusCode::INTERNAL_SERVER_ERROR)?;
    let digest = blake3::hash(&bytes);
    let sig = sk.sign(digest.as_bytes());
    grant.seal = GrantSeal {
...
fn verify_grant(grant: &AccessGrant, pk_b64: &str) -> Result<(), axum::http::StatusCode> {
    ...
    let sig_b = general_purpose::STANDARD
        .decode(to_verify.seal.sig.as_bytes())
        .map_err(|_| axum::http::StatusCode::UNAUTHORIZED)?;
...
fn paseto_sign(grant: &AccessGrant, sk_b64: &str) -> Result<String, axum::http::StatusCode> {
    ...
    let msg =
        serde_json::to_vec(grant).map_err(|_| axum::http::StatusCode::INTERNAL_SERVER_ERROR)?;
...
fn paseto_verify(token: &str, pk_b64: &str) -> Result<AccessGrant, axum::http::StatusCode> {
    ...
    let grant: AccessGrant =
        serde_json::from_slice(msg).map_err(|_| axum::http::StatusCode::UNAUTHORIZED)?;
...
static GRL_CACHE: once_cell::sync::Lazy<std::sync::Mutex<(std::time::Instant, Vec<u8>)>> =
    once_cell::sync::Lazy::new(|| std::sync::Mutex::new((std::time::Instant::now(), Vec::new())));
...
async fn well_known_grl() -> Result<impl IntoResponse, axum::http::StatusCode> {
    ...
    if let Ok(remote) = std::env::var("GRL_REMOTE_URL") {
        let ttl_ms: u64 = std::env::var("GRL_TTL_MS")
            .ok()
        ...
        if !data.is_empty() {
            if let Ok(remote_v) = serde_json::from_slice::<serde_json::Value>(data) {
                ...
                if let (Some(local_g), Some(remote_g)) =
                    (merged.get_mut("grants"), remote_v.get("grants"))
                {
                    if let (Some(local_arr), Some(remote_arr)) =
                        (local_g.as_array_mut(), remote_g.as_array())
                    {
                        for r in remote_arr {
                            if !local_arr.contains(r) {
                                local_arr.push(r.clone());
                            }
                        }
                    }
                }
    ...
    let body =
        serde_json::to_vec(&merged).map_err(|_| axum::http::StatusCode::INTERNAL_SERVER_ERROR)?;
    Ok((
...
async fn health_handler() -> Result<impl IntoResponse, axum::http::StatusCode> {
    ...
    let _ = emit_audit_report(
        "./tenants/_public",
        "health",
        &serde_json::json!({}),
        &status,
        &serde_json::json!({"intent":"health"}),
    )
    .await;
    Ok((
        [(axum::http::header::CONTENT_TYPE, "application/json")],
...
async fn presign_handler(
...
) -> Result<Json<PresignResp>, axum::http::StatusCode> {
    if std::env::var("PRESIGN_DISABLE").ok().as_deref() == Some("1") {
    ...
    match &pd {
        PolicyDecision::NACK { error } => {
            ...
            let _ = emit_audit_report(
                "./tenants/_public",
                req.who.as_deref().unwrap_or("anonymous"),
                &meta,
                &out,
                &serde_json::json!({"intent":"presign"}),
            )
            .await;
            return Ok(Json(PresignResp {
                url: "".into(),
                meta: out,
            }));
        }
    ...
    let url = match req.backend.as_str() {
        "fs" => {
            #[cfg(feature = "fs")]
            {
                ...
                if req.verb == "GET" {
                    reg.presign_get(&req.bucket, &req.key, req.ttl_secs).await
                } else {
                    reg.presign_put(&req.bucket, &req.key, req.ttl_secs).await
                }
            }
            #[cfg(not(feature = "fs"))]
            {
                Err(anyhow::anyhow!("fs backend not compiled"))
                    .map_err(|_| axum::http::StatusCode::BAD_REQUEST)?
            }
        }
        "s3" => {
            #[cfg(feature = "s3")]
            {
                let reg = engine_registry::s3_registry::S3Registry::new_from_env()
                    .await
                    .map_err(|_| axum::http::StatusCode::INTERNAL_SERVER_ERROR)?;
                if req.verb == "GET" {
                    reg.presign_get(&req.bucket, &req.key, req.ttl_secs).await
            ...
            #[cfg(not(feature = "s3"))]
            {
                Err(anyhow::anyhow!("s3 backend not compiled"))
                    .map_err(|_| axum::http::StatusCode::BAD_REQUEST)?
            }
        }
        _ => {
    ...
    let grant = AccessGrant {
        ...
        seal: GrantSeal {
            alg: "ed25519-blake3".into(),
            kid: "dev-2026-key-1".into(),
            sig: "".into(),
        },
    };
    let sk_b64 = std::fs::read_to_string("secrets/dev/ed25519.private.b64").unwrap_or_default();
    let grant = sign_grant(grant, &sk_b64, "dev-2026-key-1")
        .map_err(|_| axum::http::StatusCode::INTERNAL_SERVER_ERROR)?;
    let grant_json = serde_json::to_value(&grant).unwrap();
    let paseto_token =
        paseto_sign(&grant, &sk_b64).map_err(|_| axum::http::StatusCode::INTERNAL_SERVER_ERROR)?;
    ...
    let _ = emit_audit_report(
        ...
        who,
        &meta,
        &serde_json::json!({"presigned_url": url, "grant": grant_json, "paseto": paseto_token}),
        &serde_json::json!({"intent":"presign"}),
    )
    .await;
...
fn check_revocation_signed(grant_id: &str) -> bool {
    ...
    if mf.kind != "revocation.manifest.v1" {
        return false;
    }
    ...
    if vk.verify(digest.as_bytes(), &sig).is_err() {
        return false;
    }
...
async fn s3_proxy_handler(
    headers: HeaderMap,
    Query(q): Query<HashMap<String, String>>,
) -> Result<impl IntoResponse, axum::http::StatusCode> {
    if std::env::var("PROXY_DISABLE").ok().as_deref() == Some("1") {
        return Err(axum::http::StatusCode::SERVICE_UNAVAILABLE);
    }
    // Expect header X-LogLine-Grant: base64(JSON)
    let grant_b64 = headers
        .get("X-LogLine-Grant")
        .ok_or(axum::http::StatusCode::UNAUTHORIZED)?;
    let grant_bytes = base64::engine::general_purpose::STANDARD
        .decode(grant_b64.as_bytes())
        .map_err(|_| axum::http::StatusCode::UNAUTHORIZED)?;
    let grant: AccessGrant =
        serde_json::from_slice(&grant_bytes).map_err(|_| axum::http::StatusCode::UNAUTHORIZED)?;
    ...
    let exp = chrono::DateTime::parse_from_rfc3339(&grant.exp)
        .map_err(|_| axum::http::StatusCode::UNAUTHORIZED)?
    ...
    #[cfg(feature = "s3")]
    {
        ...
        let _ = emit_audit_report(
            ...
            &meta,
            &serde_json::json!({"intent":"proxy_get"}),
        )
        .await;
        return Ok((
