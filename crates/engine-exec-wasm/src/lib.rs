...
impl WasmExecutor {
    ...
    fn validate(&self, bytes: &[u8]) -> Result<()> {
        if !self.cfg.allow_imports {
            for p in Parser::new(0).parse_all(bytes) {
                if let Payload::ImportSection(_) = p? {
                    bail!("imports not allowed in deterministic mode");
                }
            }
        }
        Ok(())
    ...
    pub fn exec(&self, unit: &[u8], input: &Json) -> Result<Vec<u8>> {
        ...
        let memory = instance
            ...
            .ok_or_else(|| anyhow!("export memory required"))?;
        let alloc: TypedFunc<i32, i32> = instance
            .get_typed_func(&mut store, "alloc")
            .map_err(|_| anyhow!("export alloc required"))?;
        let dealloc: TypedFunc<(i32, i32), ()> = instance
            .get_typed_func(&mut store, "dealloc")
            .map_err(|_| anyhow!("export dealloc required"))?;
        let run: TypedFunc<(i32, i32), (i32, i32)> = instance
            .get_typed_func(&mut store, "run")
            .map_err(|_| anyhow!("export run required"))?;
        if memory.data_size(&store) > self.cfg.memory_limit_bytes {
            bail!("guest memory exceeds limit");
        }
        if in_bytes.len() > self.cfg.memory_limit_bytes {
            bail!("input exceeds limit");
        }
        let in_ptr = alloc.call(&mut store, in_bytes.len() as i32)?;
        let data = memory.data_mut(&mut store);
        let s = in_ptr as usize;
        let e = s + in_bytes.len();
        if e > data.len() {
            bail!("bad alloc OOB");
        }
        ...
        let s2 = out_ptr as usize;
        let e2 = s2 + (out_len as usize);
        if e2 > data2.len() {
            bail!("guest OOB");
        }
