...
use engine_extras::expr_registry::{BasicRegistry, ExtensibleExpr};
...
fn run_example(input_path: &str, outdir: &str, k: usize, sign_pem: &Option<String>) -> Result<()> {
    ...
    let pa = PolicyBit::new("has_role", "actor has role")
        ...
        .condition(Expression::eq(
            Expression::context(&["actor", "role"]),
            Expression::literal("admin"),
        ))
        .build();
    let pb = PolicyBit::new("has_quota", "quota > 0")
        .requires(&["actor", "quota"])
        .condition(Expression::gt(
            Expression::context(&["actor", "quota"]),
            Expression::literal(0),
        ))
        .build();
    let pc = PolicyBit::new("resource_ok", "resource not restricted")
        .requires(&["resource", "restricted"])
        .condition(Expression::not(Expression::context(&[
            "resource",
            "restricted",
        ])))
        .build();
    ...
    let unit = engine_core::AtomicUnit::builder("unit_required")
        ...
        .policy(pc)
        .wiring(Wiring::All {
            policies: vec!["has_role".into(), "has_quota".into(), "resource_ok".into()],
        })
        .build();
...
fn main() -> Result<()> {
    ...
    match args.cmd {
        Cmd::Run {
            ...
            k,
            sign_pem,
        } => run_example(&input, &outdir, k, &sign_pem),
        Cmd::RegistryPut {
            name,
            version,
            cid,
            regdir,
        } => reg_put(&name, &version, &cid, &regdir),
    }
}
