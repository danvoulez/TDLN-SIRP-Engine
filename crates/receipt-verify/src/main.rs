...
fn main() -> Result<()> {
    ...
    if args.is_empty() {
        eprintln!("usage: receipt-verify <receipt_card.json> [--pubkey pub.pem]");
        std::process::exit(2);
    }
    ...
    let chain = proof
        .get("hash_chain")
        .ok_or_else(|| anyhow!("missing hash_chain"))?
        .as_array()
        .ok_or_else(|| anyhow!("hash_chain not array"))?;
    ...
    let output_cid = card
        ...
        .unwrap_or("");
    println!(
        "✅ receipt OK  | input CID: {input_cid} | output CID: {output_cid} | chain_len: {}",
        chain.len()
    );
    Ok(())
}
